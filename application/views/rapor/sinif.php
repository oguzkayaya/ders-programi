...
<body class="hold-transition skin-yellow-light sidebar-mini sidebar-collapse">
    ...
    <?php $viewD["baslik"] = "Sınıf Ders Programı";
    ...
    <section class="content"  style="text-align:center;">
        <div class="col-md-12" style="height:125px;">
            ...
            <div class="col-md-4">
                <select class="form-control sBox" id="sinifS"
                onchange="
                    document.getElementById('gosB').disabled = false;
                ">
                    <option disabled selected hidden value="-1">Sınıf Seçiniz...</option>
                    <?php foreach ($siniflar as $sinif) { ?>
                        <option value="<?php echo $sinif->sinifId; ?>" <?php if($sinifId == $sinif->sinifId) echo "selected"; ?>><?php echo "$sinif->sinifAd"; ?></option>
                    <?php } ?>
                </select>
                <br>
                ...
                onclick="
                    var sinifId = document.getElementById('sinifS').value;
                    location.href = '<?php echo base_url("rapor/sinif/"); ?>' + sinifId;
                ">
            </div>
        </div>
    ...
    <script>
        <?php foreach ($program as $prog) { 
            ...
            '<div>' +
            '$prog->birim <br> $prog->bolum $bTur <br> $prog->dersKodu - $prog->dersAd $tur <br> $prog->unvan - $prog->elemanAd '+
            '</div>' ;
            "; 
